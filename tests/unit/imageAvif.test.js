...
describe('GET /v1/fragments/:id.ext - AVIF image conversions', () => {
  let fragmentId;
  const user = { email: 'user1@email.com', password: 'password1' };
  const imagePath = path.join(__dirname, '../images/test.avif'); // Ensure this image exists
  ...
  conversions.forEach(({ ext, contentType }) => {
    test(`Convert AVIF to .${ext}`, async () => {
      const res = await request(app)
